...
bool HexBoard::addPiece(int x, int y, char c)
{
    ...
    lastChar = c;
    remainingMoves.erase(find(remainingMoves.begin(), remainingMoves.end(), pair<int, int>(x, y)));
...
void HexBoard::clearBoard()
{
    ...
    for (int i = 1; i <= size; i++)
    {
        ...
        {
            remainingMoves.push_back(pair<int, int>(i, j));
        }
    }
...
void HexBoard::randomFill(int seed = 0)
{
    int x, y, xpos, ypos;
    ...
    char c = lastChar;
        ...
        // randomly fills until no moves remain
    while (!remainingMoves.empty())
    {
        if (lastChar == 'R') c = 'B';
        else c = 'R';

        int ind = (rand() % (remainingMoves.size()));

        x = remainingMoves[ind].first;
        y = remainingMoves[ind].second;
        ...
        lastChar = c;
        remainingMoves.erase(remainingMoves.begin() + ind);

    }
}

  // checks to see if c has won
bool HexBoard::checkWinner(char c)
{
    if (c == 'B') victor = checkWinnerB();
    if (c == 'R') victor = checkWinnerR();
    if (victor != '0') win = true;
...
void HexBoard::setTo(HexBoard h)
{
    win = h.win;                                               // Is there a winner?
    victor = h.victor;                                              // Stores char of whoever won. '0' in place of null
    lastChar = h.lastChar;
    checked = h.checked;                                 // Stores values of path walked
    remainingMoves = h.remainingMoves;                          // Stores remaining moves
    size = h.size;
