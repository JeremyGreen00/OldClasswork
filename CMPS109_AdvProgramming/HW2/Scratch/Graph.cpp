...
Graph::Graph(int V_num=50, double density=0.2, double L_range=1, double U_range=10)
{
	edge_index = new int*[V_num];
	for (int i = 0; i < V_num; ++i)
		edge_index[i] = new int[V_num];
	...
	for (int i = 0; i < max_edges*density; i++)
	{
		...
		Edge e = Edge(vecs[x], vecs[y], (rand() % int(U_range - L_range))*1.1 + L_range);
...
void	Graph::add(int x, int y, double v)
{
	if (!adjacent(x, y) && x!=y)
	{
...
int Graph::get_edge_index(int x, int y)
{
	...
	{
		if (y == edge[i].get_B().get_value())
		{
			if (x == edge[i].get_A().get_value())
			{
				return i;
			}
		}
		if (x == edge[i].get_B().get_value())
		{
			if (y == edge[i].get_A().get_value())
			{
				return i;
			}
		}
	}
	//if (edge_index[x][y] != NULL) return edge_index[x][y];
	return -1;
}

void Graph::fill_Graph(int range = 1)
{
	for (int i=0; i<vecs.size();i++)
	{
