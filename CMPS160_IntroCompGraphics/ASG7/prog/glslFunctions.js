...
function send2DTextureToGLSL(val, textureUnit, uniformName) {
  ...
  var glTextureUnit = eval("gl.TEXTURE" + textureUnit)
...
function create2DTexture(imgPath, magParam, minParam, wrapSParam, wrapTParam, callback) {
  ...
  for (var i = 0; i < scene.available_txtrs.length; i++)
  {
    if(imgPath == scene.available_txtrs[i])
    {
      callback(scene.txtrs[i])
      return
    }
  }
  ...
  image.onload = function(){
    ...
    scene.available_txtrs.push(imgPath)
    scene.txtrs.push(texture)
...
function sendAttributeBufferToGLSL(data, dataCount, attribName) {
  ...
  gl.vertexAttribPointer(a_pos, data.length/dataCount, gl.FLOAT, false, 0, 0)
